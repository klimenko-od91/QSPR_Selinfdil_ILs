df_test <- read.csv("datafolder/Selectivity_external_for_SQL.csv", 
 header=TRUE, check.names=FALSE,
                 as.is=TRUE,  row.names=1) #need to put on a server

k = 33 #best model

df_pred <- read.csv(paste("scriptfolder/BIDAC/80BIDAC/", k, "_external_ts_pred_BIDAC.csv", sep = "" ) , 
 header=TRUE, check.names=FALSE,
...
### removing results for 2 IL present in Paduz.
...
write.csv(best_results_list, file = "scriptfolder/BIDAC/80BIDAC/33_stat_external.csv")
