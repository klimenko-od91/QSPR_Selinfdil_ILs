...
df_pred <- read.csv(paste("scriptfolder/BIDAC/20BIDAC/", k, "_ts_pred_BIDAC.csv", sep = "" ) , 
 header=TRUE, check.names=FALSE,
...
write.csv(best_results_list, file = "scriptfolder/BIDAC/20BIDAC/best_results_list.csv")

		} 

which(best_results_list[, 'decision'] == max(best_results_list[, 'decision'] )) #9 is the best
