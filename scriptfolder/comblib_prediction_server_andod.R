# Activate the library
library('tensorflow')
library('keras')

load("datafolder/more_az_descr.R")
sss <- vector("numeric", length = nrow(descr_new_az_all))
descr_new_az_all <- cbind(sss, descr_new_az_all)
df_test <- as.matrix(descr_new_az_all)

### normalizing

load(file ="datafolder/Selectivity_k.R") 
load(file ="datafolder/Selectivity_c.R")
###
for (i in 2:ncol(df_test)) {

df_test[,i] <- round(	(k[i]*df_test[,i]+c[i]),	1)

}


j = 16

### load model of interest
pred_test <- matrix(0, nrow = nrow(df_test), ncol = 5)

for (i in 1:5) {

model <- load_model_tf(paste("scriptfolder/log10S/80logS/", j,"_", i,"_","Selectivity_NN_benchmark.tf", sep = ""), c('PReLU' = layer_activation_parametric_relu), compile = TRUE)


pred_test_fold <-  predict(model,df_test[, 2:ncol(df_test)]) ###

pred_test[,i] <- pred_test_fold

}

### averaging 5-fold results
pred_test <- apply(pred_test,1,mean)
pred_test_df <- data.frame(df_test[, 1], pred_test ,df_test[, 2])

### export
write.csv(pred_test_df, file = paste("datafolder", j,"_","comblib3_pred_Selectivity.csv", sep = "") ) 

