...
df_full <- read.csv("datafolder/Selectivity_QSPR/Selectivity_df_QSPR.csv", 
...
df_pred <- read.csv(paste("scriptfolder/log10S/80logS/", k, "_ts_pred_Selectivity.csv", sep = "" ) , 
 header=TRUE, check.names=FALSE,
...
save(mae_per_comp, file = "scriptfolder/log10S/80logS/mae_per_comp.R") 
...
write.csv(best_results_list, file = "scriptfolder/log10S/80logS/best_results_list.csv")
...
Trs_MAE <- read.csv(paste("scriptfolder/log10S/80logS/", k, "_", j, "_model_fitted_min.csv", sep = "" ) , 
 header=TRUE, check.names=FALSE,
                 as.is=TRUE,  row.names=1)

Val_MAE <- read.csv(paste("scriptfolder/log10S/80logS/", k, "_", j, "_model_fitted_df.csv", sep = "" ) , 
 header=TRUE, check.names=FALSE,
                 as.is=TRUE,  row.names=1)
...
write.csv(Internal_Val_stat, file = "scriptfolder/log10S/80logS/Selectivity_16_Internal_Val_stat.csv")
...
df_pred <- read.csv(paste("scriptfolder/log10S/80logS/", k, "_ts_pred_Selectivity.csv", sep = "" ) , 
 header=TRUE, check.names=FALSE,
                 as.is=TRUE,  row.names=NULL) # this will be called differently!
...
Spec <- sum(df_test[, "lg [K]"] == "FALSE" & df_test[, "pred_test"] == "FALSE")   /   sum(df_test[, "lg [K]"] == "FALSE") # fraction of negatives: 0.8674843
BA <- (Sens+Spec)/2
...
write.csv(stat_class, file = "scriptfolder/log10S/80logS/16_stat_class_test_no_AD.csv")
